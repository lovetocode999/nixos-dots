  ...
  config = lib.mkIf config.boot.loader.zfsbootmenu.enable {
    ...
    boot.loader.external = {
      ...
      installHook = pkgs.writeShellScript "zfsbootinstall" ''
        ...
        ${pkgs.coreutils}/bin/mkdir -p ${config.boot.loader.zfsbootmenu.bootdir}
        ...
        copyToBoot() {
          ...
          local dstname=$2
          local generation=$3
          local dst=${config.boot.loader.zfsbootmenu.bootdir}/$dstname-$(kernver "$src")-g$generation
        ...
        ${pkgs.coreutils}/bin/rm ${config.boot.loader.zfsbootmenu.bootdir}/vmlinuz-* || true
        ${pkgs.coreutils}/bin/rm ${config.boot.loader.zfsbootmenu.bootdir}/initramfs-* || true
